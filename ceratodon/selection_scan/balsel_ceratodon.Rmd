...
scanc95_daf <- scanc95_daf %>% mutate(daf = cnt_derived / n)  %>% mutate(maf = cnt_minor / n)
daf <- daf %>% mutate(daf = cnt_derived / n)  %>% mutate(maf = cnt_minor / n)

mean(scanc95_daf$maf) # 0.1310996
mean(daf$maf) # 0.007428614
median(scanc95_daf$maf) # 0.0625
median(daf$maf) # 0
wilcox.test(scan95_daf$maf, daf$maf)

ggplot(scanc95_daf, aes(x = maf)) + geom_histogram(bins = 20)
ggsave("ceratodon_balsel1.pdf",width = 5,height = 5,units = c("in"))
ggplot(daf, aes(x = maf)) + geom_histogram(bins = 20)
ggsave("ceratodon_balsel2.pdf",width = 5,height = 5,units = c("in"))
