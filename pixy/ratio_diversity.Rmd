...
## read stats (eg. pi)
```{r}
pixy2 <- read.table("/Users/yuanmeng/Dropbox/Rumex_eQTL/diversity2022/pixy_pi_syn.0329.genewise.txt", header=TRUE) 
head(pixy2)
pixy2 <- pixy2[complete.cases(pixy2),]
pixy2 <- pixy2 %>% select(c(2:8))
colnames(pixy2)[1:3] <- c("chrom", "start", "end")

gff <-read.table("TE_filtered_REF_LA.gff")
colnames(gff)<-c("Geneid","chrom","start","end")

pixy2 <- left_join(pixy2, gff, by=c("chrom", "start", "end"))
ratio_pixy<- inner_join(expression_ratio,pixy2,by=c("Geneid","chrom", "start", "end"))
