...
chrom   physPos ref     alt     cnt_minor       cnt_alt cnt_ref n       ref_pileup      outgroup        cnt_derived
...
scan95_daf <- scan95_daf %>% mutate(daf = cnt_derived / n)
scan95_daf <- scan95_daf %>% mutate(maf = cnt_minor / n)

daf <- daf %>% mutate(daf = cnt_derived / n)  %>% mutate(maf = cnt_minor / n)

p1 <- ggplot(scan95_daf, aes(x = maf)) + geom_histogram(bins = 20)
p2 <- ggplot(daf, aes(x = maf)) + geom_histogram(bins = 20)
p1 + p2
ggsave(paste0(data, "rumex_balsel.pdf"))
mean(scan95_daf$maf) # 0.02131347
mean(daf$maf) # 0.01811379
median(scan95_daf$maf)
median(daf$maf)
wilcox.test(scan95_daf$maf, daf$maf)
```
