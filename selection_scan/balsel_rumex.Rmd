...
lines_data_facet_A3 <- data.frame(xintercept = c(11.5,31.8), facet = "A3")
...
inversion_A3 <- scan95 %>% filter(chrom == "A3" & physPos_mb > 11.5 & physPos_mb < 31.8) # 11132
...
inversion <- inversion %>% inner_join(gff, by = "chrom") %>%
        filter(physPos >= start & physPos <= end) # 9267

result_CLR <- inversion %>%
    group_by(gene) %>%
    summarise(mean_CLR = mean(CLR), max_CLR = max(CLR), no_sites = length(CLR)) # 125
