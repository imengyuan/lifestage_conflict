...
unbiased <- result_tissue %>% filter(padj >= 0.1) # 858
...
## contigency test
(old)
category, pollen-biased, unbiased, leaf-biased
balancing_sel, 155, 35, 251
whole_genome, 8998, 4677, 11375

category, p_biased, unbiased, l_biased
balancing_sel, 142, 21, 242
whole_genome, 6823, 858, 10301
...
levels(cntsr$group) <- c("s_biased", "g_biased", "unbiased")
cntsr$group <- factor(cntsr$group, level=c("g_biased", "s_biased", "unbiased"))


hcols <- c("black", "grey" ,"white")
p1 <- ggplot(cntsr) +
geom_col(aes(x = category, y = count, fill = group), position = "fill", colour ="black")  + xlab("") + ylab("Proportion of genes") + theme_classic() +ggtitle("a") + scale_fill_manual(values = hcols) + theme(axis.text=element_text(size=15), axis.title=element_text(size=15)) + guides(fill = guide_legend(override.aes = list(size = 12)))
