...
FC_pixy<- inner_join(DE_tissue,pixy,by="gene") # 11735
...
statsr <- inner_join(FC_pixy, pixy_nonsyn, by=c("chrom", "start", "end")) # 11734
write.table(statsr, file = paste0(directory,"pi_stats_rumex.txt"), row.names = FALSE, quote = FALSE)

statsr <- statsr %>% dplyr::select(gene, avg_pi, avg_pi_nonsyn)
...
stats_genesl <- inner_join(genesl, statsr, by="gene")
stats_genesp <- inner_join(genesp, statsr, by="gene")

mean(stats_genesl$pn_ps) # 0.5975733
mean(stats_genesp$pn_ps) # 0.7590268

unbiased <- FC_pixy %>% filter(padj>0.1) # 532
stats_unbiased <- inner_join(unbiased, statsr, by="gene") # 500
mean(stats_unbiased$pn_ps) # 0.737404
mean(statsr$pn_ps)
# box plots
sd(stats_genesl$pn_ps) # 0.5975733
sd(stats_genesp$pn_ps) # 0.7590268
sd(stats_unbiased$pn_ps) # 0.737404
sd(statsr$pn_ps)
...
```{r}
ggplot(statsr, aes(x=baseMean, y=pn_ps)) + geom_point(colour="#1F78B4", alpha=0.2) + geom_smooth(color="black") + theme_classic() + theme(text = element_text(size = 10)) + xlim(0,1000)

ggplot(statsr, aes(x=l_mean, y=pn_ps)) + geom_point(colour="#1F78B4", alpha=0.2) + geom_smooth(color="black") + theme_classic() + theme(text = element_text(size = 10)) + xlim(0,1000)

ggplot(statsr, aes(x=p_mean, y=pn_ps)) + geom_point(colour="#1F78B4", alpha=0.2) + geom_smooth(color="black") + theme_classic() + theme(text = element_text(size = 10)) + xlim(0,1000)
```
